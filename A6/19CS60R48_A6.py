...
def p_page(p):
	'Start : S_PAGE ALL E_PNAME'
	global page
	pnum=str(p[2])
	#print(pnum,end="->")
	try:
		page_num_list=[int(x) for x in re.findall('\d+', pnum)]
		pnum=int(page_num_list[1]) - int(page_num_list[0])
	except:
		try:
			pnum=int(pnum)
		except:
	...
	pnum=abs(pnum)+1
	#print(pnum)
	while len(author_name)-1>len(page):
		page.append(0)
	page.append(pnum)

def p_volume(p):
	'Start : V_START ALL E_PNAME'
	global volumeNumber
	while len(author_name)-1>len(volumeNumber):
		volumeNumber.append(" ")
	volumeNumber.append(str(p[2]))

def p_number(p):
	'Start : N_START ALL E_PNAME'
	global issueNumber
	while len(author_name)-1>len(issueNumber):
		issueNumber.append(" ")
	issueNumber.append(str(p[2]))
...
def main():
	...
	for prof in htmlFileNames:
		with open(prof, 'r') as myfile:
			...
			parser.parse(data)
	# print("All entries")
	while len(author_name)>len(page):
		page.append(0)
	while len(author_name)>len(issueNumber):
		issueNumber.append("")
	while len(author_name)>len(volumeNumber):
		volumeNumber.append("")
	# for i in range(len(author_name)):
	#  	print(f'{author_name[i]} | {paper_type[i]} | {paper_year[i]} | {paper_name[i]} | {paper_venue[i]} | {page[i]}')
	print(volumeNumber)
	print(issueNumber)
	while True:
		flag=0
		...
		with open('conferenceList.txt') as f:
			venueTemp = f.readlines()
		venueTemp = [x.strip() for x in venueTemp]
		venueTemp = list(set(venueTemp))
		for x in venueTemp:
			for i in range(len(author_name)):
				if(x.casefold()==paper_venue[i].casefold()):
					selectedrow[i]=selectedrow[i]+1
		print("******Query I*******")
		for i in range(len(selectedrow)):
			if selectedrow[i]==2:
				...
				print(f'Page Count:{page[i]}')
				print(f'Volume:{volumeNumber[i]}')
				print(f'Issue:{issueNumber[i]}')
				print("-------------------------------------------------------------")
				flag=1
		...
		for i in range(len(selectedrow)):
			if selectedrow[i]==3:
				...
				if len(selectedrow)==0 or flag==0:
					...
					continue
		print("******Query III*******")
		print("The number of top conferences full paper: ",count)
